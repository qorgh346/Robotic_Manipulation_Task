...
class PositionIKRequest {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.ik_link_names = null;
      this.pose_stamped_vector = null;
      this.timeout = null;
      this.attempts = null;
    }
    else {
      ...
      if (initObj.hasOwnProperty('ik_link_names')) {
        this.ik_link_names = initObj.ik_link_names
      }
      else {
        this.ik_link_names = [];
      }
      if (initObj.hasOwnProperty('pose_stamped_vector')) {
        this.pose_stamped_vector = initObj.pose_stamped_vector
      }
      else {
        this.pose_stamped_vector = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize message field [ik_link_names]
    bufferOffset = _arraySerializer.string(obj.ik_link_names, buffer, bufferOffset, null);
    // Serialize message field [pose_stamped_vector]
    // Serialize the length for message field [pose_stamped_vector]
    bufferOffset = _serializer.uint32(obj.pose_stamped_vector.length, buffer, bufferOffset);
    obj.pose_stamped_vector.forEach((val) => {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    // Deserialize message field [ik_link_names]
    data.ik_link_names = _arrayDeserializer.string(buffer, bufferOffset, null)
    // Deserialize message field [pose_stamped_vector]
    // Deserialize array length for message field [pose_stamped_vector]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.pose_stamped_vector = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.pose_stamped_vector[i] = geometry_msgs.msg.PoseStamped.deserialize(buffer, bufferOffset)
    }
  ...
  static getMessageSize(object) {
    ...
    object.ik_link_names.forEach((val) => {
      length += 4 + val.length;
    });
    object.pose_stamped_vector.forEach((val) => {
  ...
  static md5sum() {
    ...
    return '9936dc239c90af180ec94a51596c96f2';
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.ik_link_names !== undefined) {
      resolved.ik_link_names = msg.ik_link_names;
    }
    else {
      resolved.ik_link_names = []
    }

    if (msg.pose_stamped_vector !== undefined) {
      resolved.pose_stamped_vector = new Array(msg.pose_stamped_vector.length);
      for (let i = 0; i < resolved.pose_stamped_vector.length; ++i) {
        resolved.pose_stamped_vector[i] = geometry_msgs.msg.PoseStamped.Resolve(msg.pose_stamped_vector[i]);
      }
    }
    else {
      resolved.pose_stamped_vector = []
    }
