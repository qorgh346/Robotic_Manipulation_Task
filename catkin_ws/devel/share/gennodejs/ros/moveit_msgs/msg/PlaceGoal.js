...
class PlaceGoal {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.attached_object_name = null;
      this.place_locations = null;
      this.place_eef = null;
      ...
      if (initObj.hasOwnProperty('attached_object_name')) {
        this.attached_object_name = initObj.attached_object_name
      }
      else {
        this.attached_object_name = '';
      }
      if (initObj.hasOwnProperty('place_locations')) {
        this.place_locations = initObj.place_locations
      }
      else {
        this.place_locations = [];
      }
      if (initObj.hasOwnProperty('place_eef')) {
        this.place_eef = initObj.place_eef
      }
      else {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize message field [attached_object_name]
    bufferOffset = _serializer.string(obj.attached_object_name, buffer, bufferOffset);
    // Serialize message field [place_locations]
    // Serialize the length for message field [place_locations]
    bufferOffset = _serializer.uint32(obj.place_locations.length, buffer, bufferOffset);
    obj.place_locations.forEach((val) => {
      bufferOffset = PlaceLocation.serialize(val, buffer, bufferOffset);
    });
    // Serialize message field [place_eef]
    bufferOffset = _serializer.bool(obj.place_eef, buffer, bufferOffset);
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new PlaceGoal(null);
    // Deserialize message field [group_name]
    data.group_name = _deserializer.string(buffer, bufferOffset);
    // Deserialize message field [attached_object_name]
    data.attached_object_name = _deserializer.string(buffer, bufferOffset);
    // Deserialize message field [place_locations]
    // Deserialize array length for message field [place_locations]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.place_locations = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.place_locations[i] = PlaceLocation.deserialize(buffer, bufferOffset)
    }
    // Deserialize message field [place_eef]
    data.place_eef = _deserializer.bool(buffer, bufferOffset);
  ...
  static getMessageSize(object) {
    ...
    length += object.attached_object_name.length;
    object.place_locations.forEach((val) => {
      length += PlaceLocation.getMessageSize(val);
    });
    length += object.support_surface_name.length;
  ...
  static Resolve(msg) {
    ...
    const resolved = new PlaceGoal(null);
    ...
    if (msg.attached_object_name !== undefined) {
      resolved.attached_object_name = msg.attached_object_name;
    }
    else {
      resolved.attached_object_name = ''
    }

    if (msg.place_locations !== undefined) {
      resolved.place_locations = new Array(msg.place_locations.length);
      for (let i = 0; i < resolved.place_locations.length; ++i) {
        resolved.place_locations[i] = PlaceLocation.Resolve(msg.place_locations[i]);
      }
    }
    else {
      resolved.place_locations = []
    }

    if (msg.place_eef !== undefined) {
      resolved.place_eef = msg.place_eef;
    }
    else {
      resolved.place_eef = false
    }
