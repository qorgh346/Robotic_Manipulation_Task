  ...
  as_(nh_, action_name_, false)
{
  ...
  joint_command_pub=nh_.advertise<sensor_msgs::JointState>("/j2n6s300/arm_set", 1);

  joint_command_msg.name.resize(6);
  joint_command_msg.position.resize(6);

  joint_command_msg.header.frame_id="root";
  
  for(int i=0; i<6; i++){
    joint_command_msg.name[i] = "j2n6s300_joint_" +std::to_string(i+1);
  }
  as_.start();
  ...
  as_(nh_, action_name_, false)
{
  ...
  joint_command_pub=nh_.advertise<sensor_msgs::JointState>("/j2n6s300/finger_set", 100);

  joint_command_msg.name.resize(3);
  joint_command_msg.position.resize(3);

  joint_command_msg.header.frame_id="root";
  joint_command_msg.name[0]="j2n6s300_joint_finger_1";
  joint_command_msg.name[1]="j2n6s300_joint_finger_2";
  joint_command_msg.name[2]="j2n6s300_joint_finger_3";
...
void GripperController::gGoalCB(){
  ...
  joint_command_msg.position[2] = goal_->command.position;
  joint_command_pub.publish(joint_command_msg);
