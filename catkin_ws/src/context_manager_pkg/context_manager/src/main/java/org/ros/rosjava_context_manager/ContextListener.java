...
public class ContextListener extends AbstractNodeMain {
  ...
  static int[][] oIdCount = new int[50][2];
	...
	public void onStart(ConnectedNode connectedNode) {
		...
		Subscriber<std_msgs.Float32MultiArray> object_subscriber = connectedNode.newSubscriber("/sim_ros_interface/object_info",
        ...
        */
        ...
        Subscriber<std_msgs.Float32MultiArray> robot_hand_subscriber = connectedNode.newSubscriber("/sim_ros_interface/visual_robot_perception",
		...
		Subscriber<sensor_msgs.JointState> robot_joint_subscriber = connectedNode
				.newSubscriber("/j2n6s300/joint_states_cm", sensor_msgs.JointState._TYPE);
...
/*
		...
		*/
		...
		object_subscriber.addMessageListener(new MessageListener<std_msgs.Float32MultiArray>() {
			...
			public void onNewMessage(std_msgs.Float32MultiArray message) {
				...
				float alpha = message.getData()[3+k];
				float beta = message.getData()[4+k];
				float gamma = message.getData()[5+k];
				...
				float a = (float) Math.toDegrees(alpha);
				float b = (float) Math.toDegrees(beta);
				float c = (float) Math.toDegrees(gamma);
        ...
        */
        ...
        robot_hand_subscriber.addMessageListener(new MessageListener<std_msgs.Float32MultiArray>() {
			@Override
			public void onNewMessage(std_msgs.Float32MultiArray message) {
				Quaternion quaternion_robot = new Quaternion(0, 0, 0, 0);
				...
				float alpha = message.getData()[3];
				float beta = message.getData()[4];
				float gamma = message.getData()[5];

				float finger_x = message.getData()[6];
				float finger_y = message.getData()[7];
				float finger_z = message.getData()[8];
				String ID = "";
				...
				"  Object_name:" + ID + ", Object Pose(X: " + x + " Y: "+ y+ " Z: " + z + "),Object Orientation(A: " + 
				a+ " B: " + b+ " G: " + c + " finger x : " + finger_x + " finger y : " + finger_y + " finger z : " + finger_z;
				//System.out.print(result);
			}
		},queueSize);
