...
currentObjectBox(Box, Object) :-
    ...
    >=( (IZ), (OZ - 1.5*OH) ), =<( (IZ), (OZ + 1.5*OH) ),

    InnerObj \= OuterObj,
    rdf(VPO, 'http://knowrob.org/kb/knowrob.owl#objectActedOn', Box),
    rdf(VPI, 'http://knowrob.org/kb/knowrob.owl#objectActedOn', Object).


%190807

standing_can(Object):-
   rdfs_individual_of(Object,  knowrob:'DrinkingCan'),
   currentObjectPose(Object, Pose),
   nth0(3, Pose, Tilted_Radius1),
   nth0(4, Pose, Tilted_Radius2),
   once(
   (Tilted_Radius1 =< -1, Radius1 is 0);
   (Tilted_Radius1 > -1, Tilted_Radius1 < 1, Radius1 is 1);
   (Tilted_Radius1 >= 1, Radius1 is 0)
   ),
   once(
   (Tilted_Radius2 =< -1, Radius2 is 0);
   (Tilted_Radius2 > -1, Tilted_Radius2 < 1, Radius2 is 1);
   (Tilted_Radius2 >= 1, Radius2 is 0)
   ),
   (Radius1 == 1, Radius2 == 1).
...
tilted_can(Object):-
   ...
   once(
   (Radius1 == 1, Radius2 == 1, Tmp is 1);
   (Radius1 == 0, Radius2 == 1, Tmp is 0);
   (Radius1 == 1, Radius2 == 0, Tmp is 0);
   (Radius1 == 0, Radius2 == 0, Tmp is 0)
   ),
   (Tmp == 0).
